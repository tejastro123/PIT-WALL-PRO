...
export default function AnalyticsPage() {
  ...
  // Primary Data Queries using standalone hooks
  const seasonResultsQuery = useF1SeasonResults(year);
  const seasonResults = useMemo(() => seasonResultsQuery.data || [], [seasonResultsQuery.data]);

  // Derived state: find current round
  const currentRound = useMemo(() =>
    seasonResults.length ? Math.max(...seasonResults.map((r) => r.round)) : 0
    , [seasonResults]);

  const lapDistQuery = useF1LapDistribution(year, event);
  const lapDist = lapDistQuery.data || [];

  const teamPaceQuery = useF1TeamPace(year, event);
  const teamPace = teamPaceQuery.data || [];

  const summaryQuery = useF1SeasonSummary(year);
  const standings = summaryQuery.data || [];

  const wdcQuery = useF1WdcScenarios(year, currentRound);
  const wdcScenarios = wdcQuery.data;

  const wccQuery = useF1WccScenarios(year, currentRound);
  const wccScenarios = wccQuery.data;

  // const loading = seasonResultsQuery.isLoading || lapDistQuery.isLoading || teamPaceQuery.isLoading;
  ...
  const seasonPointsData = useMemo(() => {
    ...
    return rounds.map(rnd => {
      const data: Record<string, string | number> = { round: `R${rnd}` };
      drivers.forEach(drv => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="card-base p-6 border-l-4 border-l-yellow-500 bg-yellow-500/5">
          ...
          <div className="font-orbitron font-black text-2xl text-white tracking-tighter">
            {(wdcScenarios?.drivers || []).filter((d: WdcScenario) => d.can_win).length || 0} <span className="text-xs text-white/40 uppercase font-bold">Drivers Active</span>
          </div>
        ...
        <div className="card-base p-6 border-l-4 border-l-blue-500 bg-blue-500/5">
          ...
          <div className="font-orbitron font-black text-2xl text-white tracking-tighter">
            {(wccScenarios?.teams || []).filter((t: WccScenario) => t.can_win).length || 0} <span className="text-xs text-white/40 uppercase font-bold">Teams Active</span>
          </div>
        ...
        <div className="card-base p-6 border-l-4 border-l-[var(--f1-red)] bg-[var(--f1-red)]/5">
          ...
          <div className="font-orbitron font-black text-2xl text-white tracking-tighter">
            {seasonResults.length ? Math.max(...seasonResults.map((r: SeasonResultData) => r.round)) : 0} / 24 <span className="text-xs text-white/40 uppercase font-bold">Rounds Complete</span>
          </div>
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={100}>
                <BarChart layout="vertical" data={teamPace} margin={{ left: 40 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(val: any) => [`${Number(val || 0).toFixed(3)}s`, 'Median Lap']}
                  />
              ...
              <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={100}>
                <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  <Tooltip
                    ...
                    contentStyle={{ backgroundColor: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', fontSize: '10px' }}
                  />
                  {Array.from(new Set(lapDist.map(d => d.driver))).map((drv) => (
                    <Scatter
                      key={drv}
                      name={drv}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-4 space-y-4">
          ...
          <div className="space-y-3 overflow-y-auto max-h-[440px] pr-2 custom-scrollbar">
            {standings.map((d) => {
              const scenario = wdcScenarios?.drivers.find((s) => s.driver === d.driverCode);
