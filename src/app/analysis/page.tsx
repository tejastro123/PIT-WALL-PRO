...
import { 
  Activity, Zap, 
  BarChart3, Clock, Gauge, Map as MapIcon, Layers, Maximize2
} from "lucide-react";
...
type TabId = "speed" | "track" | "laps" | "pedals" | "comparison";
...
export default function AnalysisPage() {
  ...
  const sessionQuery = useF1Session(year, event, sessionType);
  const sessionData = sessionQuery.data;
  const loading = sessionQuery.isLoading;
  const sessionReady = !!sessionData?.results?.length;
  ...
  const activeDriver = selectedDriver || sessionData?.results?.[0]?.driver_code || null;

  const telemetryQuery = useF1Telemetry(year, event, sessionType, activeDriver);
  const telemetry = telemetryQuery.data;

  const trackQuery = useF1TrackMap(
    year,
    event,
    sessionType,
  ...
  const isFetching = telemetryQuery.isFetching || trackQuery.isFetching || lapsQuery.isFetching;

  return (
    <div className="max-w-[1700px] mx-auto px-6 md:px-12 py-10 pb-32 relative">
      ...
      <AnimatePresence>
        {isFetching && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed bottom-10 right-10 z-[100] card-base p-6 border-l-4 border-l-[var(--f1-red)] bg-black/80 backdrop-blur-xl shadow-2xl flex items-center gap-6"
          >
            <div className="relative">
              <Activity size={24} className="text-[var(--f1-red)] animate-pulse" />
              <div className="absolute inset-0 bg-[var(--f1-red)] blur-lg opacity-20 animate-pulse" />
            </div>
            <div>
              <h4 className="font-orbitron font-black text-[10px] text-white tracking-[0.3em] uppercase">Uplink_In_Progress</h4>
              <p className="font-mono text-[9px] text-white/40 uppercase mt-1">Downloading_Large_Telemetry_Set...</p>
            </div>
            <div className="flex gap-1">
              {[1,2,3].map(i => (
                <div key={i} className="w-1 h-3 bg-[var(--f1-red)]" style={{ animation: `pulse 1s ease-in-out ${i * 0.2}s infinite` }} />
              ))}
            </div>
          </motion.div>
        )}
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <span className={cn(
              "font-mono text-[10px] tracking-[0.4em] font-bold uppercase",
              isFetching ? "text-[var(--f1-red)] animate-pulse" : "text-green-500"
            )}>
              {isFetching ? "Telemetry_Link_Syncing..." : "Telemetry_Link_Established"}
            </span>
            <div className={cn(
              "h-[1px] w-12 bg-gradient-to-r to-transparent",
              isFetching ? "from-[var(--f1-red)]" : "from-green-500"
            )} />
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (activeTab === "comparison") {
                        if (activeDriver === res.driver_code) return;
                    ...
                    className={cn(
                      ...
                      (activeDriver === res.driver_code || compareDriver === res.driver_code) && "bg-white/[0.03]"
                    )}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-orbitron font-bold text-xs text-white uppercase flex items-center gap-2">
                          {res.driver_code}
                          {activeDriver === res.driver_code && <span className="text-[8px] px-1 bg-[var(--f1-red)] text-white font-black tracking-tighter">REF</span>}
                          {compareDriver === res.driver_code && <span className="text-[8px] px-1 bg-blue-500 text-white font-black tracking-tighter">CMP</span>}
                        </div>
                        <div className="font-mono text-[9px] text-white/40 uppercase tracking-tighter">{res.team}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-orbitron font-bold text-[10px] text-white">P{res.position || '--'}</div>
                      <div className="font-mono text-[8px] text-white/20 uppercase">{res.status}</div>
                    </div>
                  </button>
                ))
              )}
        ...
        <div className="xl:col-span-9 space-y-6">
          ...
          <AnimatePresence mode="wait">
            <motion.div
              key={activeTab + (activeDriver || "") + (compareDriver || "")}
              ...
              {activeTab === "speed" && (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <StatBox label="Avg RPM" value={telemetry ? Math.floor(telemetry.telemetry.rpm.reduce((a: number, b: number)=>a+b,0)/telemetry.telemetry.rpm.length) : "----"} icon={Activity} color="white" />
                  ...
                  <div className="card-base p-8">
                    <div className="flex items-center justify-between mb-8">
                      ...
                      <div className="flex gap-4 font-mono text-[9px]">
                        <span className="flex items-center gap-2"><div className="w-2 h-2" style={{ backgroundColor: getDriverColor(activeDriver || "") }} /> {activeDriver}</span>
                      </div>
                    </div>
                        ...
                        <AreaChart data={chartData}>
                          <defs>
                            <linearGradient id="colorSpeed" x1="0" y1="0" x2="0" y2="1">
                              <stop offset="5%" stopColor={getDriverColor(activeDriver || "")} stopOpacity={0.1}/>
                              <stop offset="95%" stopColor={getDriverColor(activeDriver || "")} stopOpacity={0}/>
                            </linearGradient>
                          </defs>
                          ...
                          />
                          <Area type="monotone" dataKey="speed" stroke={getDriverColor(activeDriver || "")} fillOpacity={1} fill="url(#colorSpeed)" strokeWidth={2} />
                        </AreaChart>
                      </ResponsiveContainer>
                      )}
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="card-base p-8">
                        <div className="flex items-center justify-between mb-8">
                          ...
                          <div className="flex gap-6 font-mono text-[9px]">
                            <span className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: getDriverColor(activeDriver || "") }} /> {activeDriver}</span>
                          ...
                          {!mounted ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={100}>
                              <LineChart data={compChartData}>
                                ...
                                <Line type="monotone" dataKey={activeDriver || ""} stroke={getDriverColor(activeDriver || "")} dot={false} strokeWidth={2} />
                      ...
                      <div className="card-base p-8 border-t-2 border-t-[var(--f1-red)]">
                        ...
                        <p className="mt-4 font-mono text-[8px] text-[var(--f1-gray-light)] uppercase tracking-widest text-center italic">
                          * Values above 0 indicate {activeDriver} is losing time relative to {compareDriver}
                        </p>
                      </div>
                    </div>
                  )}
                    ...
                    {!mounted ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={100}>
                        <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                          ...
                          />
                          {Array.from(new Set(allLaps.filter(l => l.driver === activeDriver).map(l => l.compound))).map((compound: string) => (
                            <Scatter 
                              key={compound}
                              name={compound} 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="card-base p-8">
                      ...
                      <div className="h-[200px] w-full flex items-end gap-1">
                        {chartData.filter((_, i) => i % 20 === 0).map((d: { gear: number }, i) => (
                          <div 
                            key={i} 
