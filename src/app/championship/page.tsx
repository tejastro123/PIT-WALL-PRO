...
import { useFastF1, type ChampionshipScenarios, type WdcScenario, type WccScenario } from "@/hooks/useFastF1";
...
export default function ChampionshipPage() {
  const { getWdcScenarios, getWccScenarios, getSeasonSummary } = useFastF1();
  const { year } = useSessionStore();
  const [activeTab, setActiveTab] = useState<"wdc" | "wcc" | "simulator">("wdc");
  const [wdcData, setWdcData] = useState<ChampionshipScenarios | null>(null);
  const [wccData, setWccData] = useState<ChampionshipScenarios | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const [wdc, wcc] = await Promise.all([
        getWdcScenarios(year, 4, signal),
        getWccScenarios(year, 4, signal),
        getSeasonSummary(year, signal)
      ]);
  ...
  const chartData = useMemo(() => {
    ...
    return (list || []).slice(0, 15).map((d) => ({
      ...d,
      label: activeTab === "wdc" ? (d as WdcScenario).driver : (d as WccScenario).team,
      gap: leaderPoints - d.points,
      possible: (d as WdcScenario | WccScenario).max_possible,
      driver: activeTab === "wdc" ? (d as WdcScenario).driver : undefined
    }));
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-12">
        ...
        <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-lg backdrop-blur-md">
          {[
            ...
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as "wdc" | "wcc")}
              className={cn(
              ...
              <ResponsiveContainer width="100%" height="100%" minHeight={100}>
                <BarChart layout="vertical" data={chartData} margin={{ left: 40, right: 40 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(val: any) => [Number(val || 0) === 0 ? "LEADER" : `-${val} PTS`, 'GAP']}
                  />
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <div className="card-base p-6 border border-dashed border-white/10">
            ...
            <p className="font-mono text-[9px] text-[var(--f1-gray-light)] uppercase leading-relaxed mb-4">
              To mathematically clinch the {activeTab === "wdc" ? "WDC" : "WCC"} at the next round, the leader needs a gap of at least {(activeScenarios?.max_remaining_points || 0) - (activeTab === "wdc" ? 26 : 44)} points.
            </p>
